...
VIM_DATA = [
    ...
    { "action": "选中当前行/进入行选择模式 (Visual Line Mode)", "keywords": ["选中", "选择", "当前行", "行", "整行", "模式", "visual", "line", "V", "shift"], "command": "V (Shift + v)" },
...
LAZYGIT_DATA = [
    { "action": "打开快捷键帮助菜单 (Help Menu)", "keywords": ["帮助", "快捷键", "菜单", "help", "keybindings", "menu", "?"], "command": "?" },
    ...
    { "action": "退出分支 commit log 视图并返回分支列表 (Exit Branch Commits)", "keywords": ["回退", "分支列表", "提交记录", "日志", "返回", "exit", "log", "branch", "commits", "esc", "q"], "command": "Esc 或 q" },
    { "action": "退出 Lazygit (Quit Lazygit)", "keywords": ["退出", "关闭", "quit", "exit", "q"], "command": "q 或 Ctrl + c" },
    { "action": "撤销上一步 Git 操作 [类似于 git reset --soft HEAD@{1}]", "keywords": ["撤销", "上一步", "undo", "last", "z", "reflog"], "command": "z (基于 reflog 撤销提交，不包含工作区修改)" },
    { "action": "重做上一步 Git 操作 [Redo via reflog]", "keywords": ["重做", "恢复", "redo", "Z"], "command": "Z (Shift + z)" },
    { "action": "执行自定义 Shell 命令 [Execute Shell Cmd]", "keywords": ["执行", "命令", "shell", "command", ":"], "command": ":" },
    { "action": "暂存/取消暂存单个文件 [等同于 git add <file> / git reset HEAD <file>]", "keywords": ["暂存", "取消暂存", "单文件", "stage", "unstage", "space", "空格", "add", "reset"], "command": "Space (空格键)" },
    { "action": "暂存/取消暂存所有文件 [等同于 git add . / git reset]", "keywords": ["暂存所有", "全选", "stage", "all", "a", "add", "reset"], "command": "a" },
    { "action": "输入提交信息并提交 [等同于 git commit -m]", "keywords": ["提交", "输入", "commit", "c"], "command": "c (输入 message 后按回车确认)" },
    { "action": "使用外部 Git 编辑器提交 [等同于 git commit]", "keywords": ["编辑器", "提交", "commit", "editor", "C"], "command": "C (Shift + c)" },
    { "action": "修改(追加)最近一次提交 [等同于 git commit --amend]", "keywords": ["追加", "修改", "最近一次", "amend", "A", "commit"], "command": "A (Shift + a)" },
    { "action": "推送当前分支到远程仓库 [等同于 git push]", "keywords": ["推送", "远程", "push", "P"], "command": "P (Shift + p)" },
    { "action": "从远程仓库拉取更新 [等同于 git pull]", "keywords": ["拉取", "更新", "pull", "p"], "command": "p" },
    { "action": "拉取获取最新更新但不合并 [等同于 git fetch]", "keywords": ["获取", "更新", "fetch", "f"], "command": "f" },
    { "action": "放弃工作区中的文件修改 [等同于 git checkout -- <file>]", "keywords": ["放弃", "丢弃", "修改", "discard", "d", "checkout", "restore"], "command": "d" },
    ...
    { "action": "查看并进入单个文件的具体更改/交互式暂存模式 [类似 git add -p]", "keywords": ["查看更改", "单文件", "修改内容", "交互式", "hunk", "line", "diff", "enter", "回车", "add -p"], "command": "Enter (回车键进入，Esc 或 q 退出返回)" },
    { "action": "查看暂存选项列表 (Stash Options)", "keywords": ["暂存", "保存修改", "stash", "options", "S"], "command": "S (Shift + s)" },
    { "action": "将所有修改进行 Git 暂存 [等同于 git stash save]", "keywords": ["暂存", "保存修改", "stash", "s", "save"], "command": "s" },
    { "action": "检出/切换选择的分支 [等同于 git checkout <branch>]", "keywords": ["检出", "切换", "分支", "checkout", "space", "空格", "switch"], "command": "Space (空格键)" },
    { "action": "创建新分支 [等同于 git checkout -b <branch>]", "keywords": ["创建", "新建", "分支", "new", "branch", "n", "checkout -b"], "command": "n" },
    { "action": "将选中的分支合并到当前分支 [等同于 git merge <branch>]", "keywords": ["合并", "分支", "merge", "M"], "command": "M (Shift + m)" },
    { "action": "在选中分支上变基当前分支 [等同于 git rebase <branch>]", "keywords": ["变基", "分支", "rebase", "r"], "command": "r" },
    { "action": "强制切换分支并丢弃本地改动 [等同于 git checkout -f <branch>]", "keywords": ["强制切换", "丢弃改动", "checkout", "force", "F"], "command": "F (Shift + f)" },
    { "action": "删除分支 [等同于 git branch -d <branch>]", "keywords": ["删除", "分支", "delete", "d", "branch -d"], "command": "d" },
    { "action": "重命名分支 [等同于 git branch -m <old> <new>]", "keywords": ["重命名", "分支", "rename", "R", "branch -m"], "command": "R (Shift + r)" },
    { "action": "分支面板内切换本地分支/远程/标签子面板 (Switch sub-tabs)", "keywords": ["远程分支", "本地分支", "标签", "子面板", "切换", "branch", "remote", "tags", "[", "]"], "command": "[ 或 ] (中括号键)" },
    { "action": "展开并查看选中的远程仓库的分支 (Expand Remote Branches)", "keywords": ["远程分支", "展开", "查看", "origin", "remote", "branches", "view", "enter", "回车"], "command": "Enter (回车键)" },
    { "action": "查看选中提交中包含的文件 [等同于 git show <commit> --stat]", "keywords": ["查看文件", "提交文件", "view", "files", "enter", "回车", "show"], "command": "Enter (回车键)" },
    { "action": "对当前面板列表项进行模糊搜索/过滤 (Filter Current List)", "keywords": ["搜索", "过滤", "查找", "定位", "filter", "search", "/"], "command": "/" },
    { "action": "按文件路径过滤提交记录历史 [等同于 git log -- <file>]", "keywords": ["过滤历史", "文件路径", "历史提交", "特定文件", "filter", "commits", "file", "path", "ctrl", "s", "log"], "command": "Ctrl + s (输入文件路径，仅显示修改该文件的提交)" },
    { "action": "将选中的提交检出为分离头指针 [等同于 git checkout <commit>]", "keywords": ["检出", "分离头", "checkout", "commit", "space", "空格"], "command": "Space (空格键)" },
    { "action": "从选中提交开始进行交互式变基 [等同于 git rebase -i <commit>]", "keywords": ["交互式变基", "变基", "rebase", "interactive", "e", "rebase -i"], "command": "e" },
    { "action": "将选中提交向下合并 [等同于 git rebase -i 中的 squash]", "keywords": ["合并", "向下合并", "squash", "s"], "command": "s" },
    { "action": "丢弃/删除选中提交 [等同于 git rebase -i 中的 drop]", "keywords": ["丢弃", "删除", "提交", "drop", "delete", "d"], "command": "d" },
    { "action": "修改选中提交的提交信息 [等同于 git rebase -i 中的 reword]", "keywords": ["修改信息", "改写", "reword", "r"], "command": "r" },
    { "action": "撤销选中提交的修改并生成新提交 [等同于 git revert <commit>]", "keywords": ["撤销", "生成提交", "revert", "t"], "command": "t" },
    { "action": "在选中提交上新建标签 [等同于 git tag <tagname> <commit>]", "keywords": ["新建标签", "标签", "tag", "T"], "command": "T (Shift + t)" },
    { "action": "复制选中的提交用作樱桃挑选 (Cherry-pick Copy)", "keywords": ["樱桃挑选", "复制", "cherry-pick", "copy", "C"], "command": "C (Shift + c)" },
    { "action": "粘贴/应用复制的樱桃挑选提交 [等同于 git cherry-pick <commit>]", "keywords": ["樱桃挑选", "粘贴", "cherry-pick", "paste", "V"], "command": "V (Shift + v)" },
    { "action": "上下移动提交顺序 [类似 git rebase -i 中调换行]", "keywords": ["移动顺序", "调整", "move", "alt", "ctrl", "j", "k"], "command": "Alt + Up / Down (或 Ctrl + k / j)" },
    { "action": "应用选中的暂存内容 [等同于 git stash apply]", "keywords": ["应用暂存", "恢复暂存", "apply", "stash", "space", "空格"], "command": "Space (空格键)" },
    { "action": "应用并删除选中的暂存内容 [等同于 git stash pop]", "keywords": ["应用删除", "弹出暂存", "pop", "stash", "g"], "command": "g" },
    { "action": "删除选中的暂存内容 [等同于 git stash drop]", "keywords": ["删除暂存", "丢弃", "drop", "stash", "d"], "command": "d" },
    { "action": "从选中的暂存内容新建分支 [等同于 git stash branch]", "keywords": ["新建分支", "暂存创建", "branch", "stash", "n"], "command": "n" },
    { "action": "解决冲突：选择当前(我们的)更改 [类似 git checkout --ours]", "keywords": ["选择当前", "解决冲突", "pick", "hunk", "space", "空格", "checkout --ours"], "command": "Space (空格键)" },
